#[
    GNU General Public License v3.0
]#

include instructions, ppu

proc loadRom (filePath: string): string =
    try: readFile filePath
    except: 
        echo getCurrentExceptionMsg()
        quit 0

let rom = loadRom "roms/tetris.gb"

const
    bios = [
        ]

    maxCycles = 69905

for i in 0..<len(bios):
    memory[i] = uint8 bios[i]

proc cpu =
    if memory[0xFF50] != 0:
        echo "success"
        for i in 0..<len(rom):
            memory[i] = uint8 rom[i]

    while (int(pc) < len rom) and (cycles*2 < maxCycles):
        execute int memory[pc]
        inc pc
        inc cycles

while true:
    cpu()
    # gpu()